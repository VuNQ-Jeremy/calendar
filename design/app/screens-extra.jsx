...
const { Card: XC, Button: XBtn, IconButton: XIB, Tag: XTag, Badge: XBadge, Switch: XSw, Avatar: XAvatar } = window.MochiDesignSystem_472b36;
const { iso } = window.MOCHI_DATE;
const ICON_TINT = window.ICON_TINT || ((color) => { const c = window.colorOf(color); return { background: c.soft, color: c.ink }; });

const MAT_TYPES = {
  notes:     { icon: 'file',   label: 'Notes',     color: 'blue' },
  worksheet: { icon: 'clipboard', label: 'Worksheet', color: 'green' },
  video:     { icon: 'video',  label: 'Video',     color: 'violet' },
  link:      { icon: 'link',   label: 'Link',      color: 'orange' },
};
...
function MaterialsScreen() {
  ...
  const className = (id) => (data.classes.find(c => c.id === id) || {}).name || 'Unfiled';
  ...
  const openNew = () => setModal({ title: '', type: 'notes', classId: data.classes[0]?.id || '', url: '', fileName: '', fileData: '', favorite: false, addedAt: iso(window.MOCHI_DATE.TODAY) });
  const save = (f) => { if (!f.title.trim()) f.title = 'Untitled material'; if (f.id) update('materials', f.id, f); else add('materials', f); setModal(null); };

  return React.createElement('div', { className: 'content' },
    React.createElement(window.PageHeader, {
      title: 'Materials', subtitle: 'Notes, worksheets, videos, and links — organized by class',
      actions: React.createElement(XBtn, { variant: 'primary', iconLeft: React.createElement(MIcon, { name: 'plus', size: 18 }), onClick: openNew }, 'Add material'),
    }),
    React.createElement('div', { className: 'cal-toolbar' },
      React.createElement('div', { style: { minWidth: 180 } }, React.createElement(window.MSelect, { value: filterClass, onChange: setFilterClass, options: [{ value: 'all', label: 'All classes' }, ...data.classes.map(c => ({ value: c.id, label: c.name }))] })),
      React.createElement('div', { style: { minWidth: 150 } }, React.createElement(window.MSelect, { value: filterType, onChange: setFilterType, options: [{ value: 'all', label: 'All types' }, ...Object.entries(MAT_TYPES).map(([k, v]) => ({ value: k, label: v.label }))] })),
      React.createElement('span', { style: { flex: 1 } }),
      React.createElement(XSw, { checked: favOnly, onChange: e => setFavOnly(e.target.checked), label: 'Favorites only' }),
    ),
    list.length ? React.createElement('div', { className: 'm-grid cols-3' },
      list.map(m => {
        const t = MAT_TYPES[m.type];
        ...
        return React.createElement(XC, { key: m.id, interactive: true },
          React.createElement('div', { className: 'm-spread', style: { alignItems: 'flex-start', marginBottom: 12 } },
            React.createElement('div', { className: 'iconwrap', style: { width: 44, height: 44, ...ICON_TINT(t.color) } }, React.createElement(MIcon, { name: t.icon, size: 20 })),
            React.createElement('button', { className: 'starbtn' + (m.favorite ? ' is-on' : ''), onClick: () => update('materials', m.id, { favorite: !m.favorite }), title: 'Favorite' }, React.createElement(MIcon, { name: m.favorite ? 'starFill' : 'star', size: 18 })),
          ),
          ...
          React.createElement('div', { className: 'lrow__meta', style: { marginBottom: 14 } },
            React.createElement('span', { className: 'mchip' }, t.label),
          ...
          React.createElement('div', { className: 'm-spread' },
            isLink
              ? React.createElement('a', { href: m.url || '#', target: '_blank', rel: 'noreferrer', className: 'm-row', style: { gap: 6, fontSize: 'var(--text-sm)', fontWeight: 700 } }, React.createElement(MIcon, { name: 'link', size: 14 }), 'Open link')
              : React.createElement('button', { className: 'm-row', style: { gap: 6, fontSize: 'var(--text-sm)', fontWeight: 700, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-link)', padding: 0, fontFamily: 'inherit' }, onClick: () => downloadMaterial(m), title: 'Download ' + (m.fileName || m.title) }, React.createElement(MIcon, { name: 'download', size: 15 }), 'Download'),
            React.createElement('div', { className: 'lrow__actions' },
              React.createElement(XIB, { label: 'Edit', size: 'sm', onClick: () => setModal({ ...m }) }, React.createElement(MIcon, { name: 'edit', size: 15 })),
              React.createElement(XIB, { label: 'Delete', size: 'sm', onClick: () => remove('materials', m.id) }, React.createElement(MIcon, { name: 'trash', size: 15 })),
            ),
          ),
        );
      }),
    ) : React.createElement(XC, null, React.createElement(window.Empty, { icon: 'folder', title: 'No materials found', sub: 'Try a different filter, or add a new material.' })),
...
function MaterialModal({ draft, setDraft, onClose, onSave, classes }) {
  ...
  return React.createElement(window.Modal, {
    open: true, onClose, title: draft.id ? 'Edit material' : 'Add material', width: 520,
    footer: React.createElement(React.Fragment, null,
      React.createElement(XBtn, { variant: 'secondary', onClick: onClose }, 'Cancel'),
      React.createElement(XBtn, { variant: 'primary', onClick: () => onSave(draft) }, 'Save'),
    ),
  },
    ...
    React.createElement('div', { className: 'm-grid cols-2', style: { gap: 14 } },
      React.createElement(window.MSelect, { label: 'Type', value: draft.type, onChange: v => set('type', v), options: Object.entries(MAT_TYPES).map(([k, v]) => ({ value: k, label: v.label })) }),
      React.createElement(window.MSelect, { label: 'Class', value: draft.classId, onChange: v => set('classId', v), options: classes.map(c => ({ value: c.id, label: c.name })) }),
    ),
    isLink
      ? React.createElement('div', { className: 'mochi-field' },
          React.createElement('label', { className: 'mochi-field__label' }, 'URL'),
          React.createElement('input', { className: 'mochi-input', placeholder: 'https://…', value: draft.url, onChange: e => set('url', e.target.value) }),
        )
      : React.createElement('div', { className: 'mochi-field' },
          React.createElement('label', { className: 'mochi-field__label' }, 'File'),
          React.createElement('label', { className: 'm-row', style: { gap: 10, padding: '14px', border: '1.5px dashed var(--border-strong)', borderRadius: 'var(--radius-md)', cursor: 'pointer', color: 'var(--text-muted)' } },
            ...
            React.createElement('span', { style: { fontSize: 'var(--text-sm)', fontWeight: 600, color: draft.fileName ? 'var(--text-strong)' : 'var(--text-muted)' } }, draft.fileName || 'Choose a file to upload'),
            React.createElement('input', { type: 'file', style: { display: 'none' }, onChange: onFile }),
          ),
          draft.fileName && React.createElement('span', { className: 'mochi-field__hint' }, draft.fileData ? 'Stored — available to download.' : 'Name saved (file too large to store in the demo).'),
        ),
  );
}
...
const PRESETS = {
  cream:    { name: 'Cream', bg: '#FFFCF8', gridLine: '#ECE0CF', today: '#FFE7D1', header: '#FDF6EC', swatches: ['#FFFCF8', '#FFE7D1', '#F79A4E'] },
  sky:      { name: 'Sky',   bg: '#F4FAFD', gridLine: '#D6ECF6', today: '#D6ECF6', header: '#ECF6FB', swatches: ['#F4FAFD', '#D6ECF6', '#57A7D2'] },
  meadow:   { name: 'Meadow', bg: '#F5FBF5', gridLine: '#D9F0DB', today: '#D9F0DB', header: '#EEF8EE', swatches: ['#F5FBF5', '#D9F0DB', '#6FB97A'] },
  lavender: { name: 'Lavender', bg: '#F8F6FE', gridLine: '#E8E0FD', today: '#E8E0FD', header: '#F4F0FE', swatches: ['#F8F6FE', '#E8E0FD', '#A185E4'] },
  dusk:     { name: 'Dusk',  bg: '#2E2A33', gridLine: '#43404B', today: '#3C3845', header: '#34303B', swatches: ['#2E2A33', '#43404B', '#A185E4'] },
};
...
function CalendarThemePanel() {
  ...
  return React.createElement('div', null,
    React.createElement('div', { className: 'mochi-eyebrow', style: { marginBottom: 8 } }, 'Presets'),
    React.createElement('div', { className: 'theme-preset', style: { marginBottom: 20 } },
      Object.entries(PRESETS).map(([key, p]) =>
        React.createElement('button', { key, className: 'preset' + (matchPreset === key ? ' is-active' : ''), onClick: () => applyPreset(key) },
          ...
          React.createElement('div', { className: 'preset__name' }, p.name),
        ),
      ),
    ),
    React.createElement('div', { className: 'mochi-eyebrow', style: { marginBottom: 4 } }, 'Fine-tune colors'),
    React.createElement(ColorRow, { k: 'bg', label: 'Canvas background', sub: 'The calendar surface' }),
    React.createElement(ColorRow, { k: 'header', label: 'Day header', sub: 'Strip above the grid' }),
    React.createElement(ColorRow, { k: 'gridLine', label: 'Grid lines', sub: 'Hairlines between cells' }),
    React.createElement(ColorRow, { k: 'today', label: 'Today highlight', sub: 'Tint on the current day' }),
    React.createElement('hr', { className: 'divider', style: { margin: '18px 0 14px' } }),
    React.createElement('div', { className: 'mochi-eyebrow', style: { marginBottom: 8 } }, 'Background image'),
    React.createElement('div', { className: 'mochi-field' },
      React.createElement('label', { className: 'mochi-field__label' }, 'Image URL'),
      React.createElement('input', { className: 'mochi-input', placeholder: 'Paste an image URL…', value: theme.bgImage, onChange: e => setTheme({ bgImage: e.target.value }) }),
    ),
    React.createElement('label', { className: 'm-row', style: { gap: 10, padding: '12px', border: '1.5px dashed var(--border-strong)', borderRadius: 'var(--radius-md)', cursor: 'pointer', color: 'var(--text-muted)', marginBottom: 14 } },
      ...
      React.createElement('span', { style: { fontSize: 'var(--text-sm)', fontWeight: 600 } }, 'Or upload an image'),
    ...
    React.createElement('div', { className: 'mochi-field' },
      React.createElement('div', { className: 'm-spread' },
        React.createElement('label', { className: 'mochi-field__label', style: { margin: 0 } }, 'Image opacity'),
    ...
    theme.bgImage && React.createElement(XBtn, { variant: 'ghost', size: 'sm', iconLeft: React.createElement(MIcon, { name: 'x', size: 15 }), onClick: () => setTheme({ bgImage: '' }) }, 'Remove image'),
  );
}
...
function ProfileScreen({ user, onSave, onLogout }) {
  ...
  return React.createElement('div', { className: 'content', style: { maxWidth: 920 } },
    React.createElement(window.PageHeader, { title: 'Your profile', subtitle: 'Manage your name, avatar, and contact details' }),
    React.createElement('div', { className: 'm-grid profile-grid', style: { gridTemplateColumns: 'minmax(0,300px) minmax(0,1fr)' } },
      ...
      React.createElement(XC, { style: { textAlign: 'center' } },
        ...
        React.createElement('div', { className: 'm-muted', style: { fontSize: 'var(--text-sm)', marginBottom: 16 } }, user.role),
        React.createElement('div', { style: { display: 'grid', placeItems: 'center' } },
          React.createElement(window.ColorPicker, { label: 'Avatar color', value: f.color, onChange: v => set('color', v) }),
        ),
      ),
      // Details card
      React.createElement('div', { className: 'm-stack', style: { gap: 20 } },
        React.createElement(XC, null,
          React.createElement('h2', { style: { margin: '0 0 16px', fontSize: 'var(--text-xl)' } }, 'Personal details'),
          React.createElement('div', { className: 'mochi-field' },
          ...
          React.createElement('div', { className: 'm-grid cols-2', style: { gap: 14 } },
            React.createElement('div', { className: 'mochi-field' },
              React.createElement('label', { className: 'mochi-field__label' }, 'Email'),
              React.createElement('input', { className: 'mochi-input', type: 'email', value: f.email, onChange: e => set('email', e.target.value) }),
            ),
            React.createElement('div', { className: 'mochi-field' },
              React.createElement('label', { className: 'mochi-field__label' }, 'Phone'),
...
window.CalendarThemePanel = CalendarThemePanel;
