...
const { Button: CBtn, IconButton: CIBtn, Tabs: CTabs, Tag: CTag, Card: CCard, Switch: CSwitch } = window.MochiDesignSystem_472b36;
...
const { iso, addDays } = window.MOCHI_DATE;
...
function EventModal({ open, onClose, draft, onSave, onDelete, classes }) {
  ...
  return React.createElement(window.Modal, {
    ...
    React.createElement('div', { className: 'm-grid cols-2', style: { gap: 14 } },
      React.createElement(window.MSelect, { label: 'Class', value: f.classId || '', onChange: v => { set('classId', v); const c = classes.find(x => x.id === v); if (c) set('color', c.color); }, options: classOpts }),
      React.createElement(window.MSelect, { label: 'Repeat', value: f.recurrence || 'none', onChange: v => set('recurrence', v), options: [{ value: 'none', label: 'Does not repeat' }, { value: 'daily', label: 'Every day' }, { value: 'weekly', label: 'Every week' }] }),
    ),
    React.createElement('div', { className: 'mochi-field' },
    ...
    React.createElement(window.ColorPicker, { label: 'Color', value: f.color || 'orange', onChange: v => set('color', v) }),
  );
}
...
function AgendaView({ cursor, events, onPick }) {
  const start = window.MOCHI_DATE.TODAY;
  ...
  if (!days.length) return React.createElement(window.Empty, { icon: 'calendar', title: 'Nothing coming up', sub: 'Your next two weeks are clear.' });
...
function CalendarScreen() {
  ...
  const [cursor, setCursor] = React.useState(() => new Date(window.MOCHI_DATE.TODAY));
  ...
  const openNew = (date, start) => setEditor({ title: '', date: date || iso(window.MOCHI_DATE.TODAY), start: start || '09:00', end: addMin(start || '09:00', 60), color: 'orange', classId: '', location: '', recurrence: 'none' });
  ...
  return React.createElement('div', { className: 'content', style: { maxWidth: 1320 } },
    ...
    React.createElement('div', { className: 'cal-toolbar' },
      ...
      React.createElement(CBtn, { variant: 'secondary', size: 'sm', onClick: () => setCursor(new Date(window.MOCHI_DATE.TODAY)) }, 'Today'),
    ...
    themeOpen && React.createElement(window.Modal, {
    ...
    }, React.createElement(window.CalendarThemePanel, null)),
  );
}
window.CalendarScreen = CalendarScreen;
window.MOCHI_CAL = { fmtTime, expandEvents, toMin, MONTHS, DOW };
