...
const { Card: SC, Button: SBtn, IconButton: SIB, Tag: STag, Badge: SBadge, Avatar: SAvatar, Checkbox: SCheck, ProgressBar: SProg } = window.MochiDesignSystem_472b36;
const { iso } = window.MOCHI_DATE;
const ICON_TINT = (color) => { const c = window.colorOf(color); return { background: c.soft, color: c.ink }; };
window.ICON_TINT = ICON_TINT;
...
function DashboardScreen({ user, onNav }) {
  ...
  const today = iso(window.MOCHI_DATE.TODAY);
  const { expandEvents, fmtTime, toMin } = window.MOCHI_CAL;
  const todays = expandEvents(data.events, window.MOCHI_DATE.TODAY, window.MOCHI_DATE.TODAY).sort((a, b) => toMin(a.start) - toMin(b.start));
  ...
  return React.createElement('div', { className: 'content' },
    React.createElement(window.PageHeader, {
      ...
      subtitle: new Date(window.MOCHI_DATE.TODAY).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' }) + ` · ${todays.length} on the calendar today`,
    }),
    ...
    React.createElement('div', { className: 'm-grid', style: { gridTemplateColumns: '1.4fr 1fr' } },
      ...
      React.createElement(SC, null,
        ...
        todays.length ? React.createElement('div', { className: 'm-stack' },
          todays.map((e, i) => {
            const c = window.colorOf(e.color);
        ...
        ) : React.createElement(window.Empty, { icon: 'calendar', title: 'Nothing scheduled', sub: 'Enjoy the quiet.' }),
      ),
      // Due today
      React.createElement(SC, null,
        React.createElement('div', { className: 'm-spread', style: { marginBottom: 16 } },
        ...
        dueToday.length ? React.createElement('div', { className: 'm-stack' },
          dueToday.map(h => {
            const c = window.colorOf(h.color);
        ...
        ) : React.createElement(window.Empty, { icon: 'check', title: 'All caught up!', sub: 'No homework due today.' }),
      ),
    ),
  );
}
...
function HomeworkScreen() {
  ...
  return React.createElement('div', { className: 'content' },
    ...
    React.createElement(window.MochiDesignSystem_472b36.Tabs, { value: filter, onChange: setFilter, tabs: [{ id: 'all', label: 'All' }, { id: 'open', label: 'Open' }, { id: 'done', label: 'Done' }] }),
    React.createElement('div', { className: 'm-stack' },
      ...
      }) : React.createElement(SC, null, React.createElement(window.Empty, { icon: 'clipboard', title: 'No tasks here', sub: 'Add homework to get started.' })),
    ),
    modal && React.createElement(window.Modal, {
      ...
      React.createElement('div', { className: 'm-grid cols-2', style: { gap: 14 } },
        React.createElement(window.MSelect, { label: 'Class', value: modal.classId, onChange: v => { const c = data.classes.find(x => x.id === v); setModal(m => ({ ...m, classId: v, color: c ? c.color : m.color })); }, options: data.classes.map(c => ({ value: c.id, label: c.name })) }),
        React.createElement('div', { className: 'mochi-field' },
