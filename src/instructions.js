    ...
    React.createElement('div', { className: 'm-stack', style: { gap: 14 } },
      FEATURES.map((f) => React.createElement('div', { key: f.key, className: 'm-row', style: { gap: 14, alignItems: 'flex-start' } },
        ...
        React.createElement('div', null,
          React.createElement('div', { style: { fontWeight: 800, color: 'var(--text-strong)' } }, t('feat_' + f.key + '_title')),
          React.createElement('div', { className: 'm-muted', style: { fontSize: 'var(--text-sm)', marginTop: 2, textWrap: 'pretty' } }, t('feat_' + f.key + '_text')),
        ),
      )),
    ),
    React.createElement('div', { className: 'm-row', style: { gap: 8, marginTop: 18, color: 'var(--text-muted)', fontSize: 'var(--text-xs)' } },
