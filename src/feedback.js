...
export function FeedbackModal({ draft, setDraft, onClose, onSave }) {
  ...
  return React.createElement(Modal, {
    open: true, onClose, title: draft.id ? t('fb_edit') : t('fb_share'), width: 520,
    footer: React.createElement(React.Fragment, null,
      React.createElement(FBtn, { variant: 'secondary', onClick: onClose }, t('cancel')),
      React.createElement(FBtn, { variant: 'primary', onClick: () => onSave(draft) }, draft.id ? t('save') : t('fb_send')),
    ),
  },
    React.createElement(MSelect, {
      label: t('fb_type'), value: draft.category, onChange: (v) => set('category', v),
    ...
    React.createElement('div', { className: 'm-grid cols-2', style: { gap: 14 } },
      React.createElement('div', { className: 'mochi-field' },
        ...
        React.createElement('input', { className: 'mochi-input', placeholder: t('auth_your_name'), value: draft.author || '', onChange: (e) => set('author', e.target.value) }),
      ),
      draft.id && React.createElement(MSelect, {
        label: t('fb_status'), value: draft.status, onChange: (v) => set('status', v),
...
export function FeedbackScreen({ user }) {
  ...
  return React.createElement('div', { className: 'content' },
    React.createElement(PageHeader, {
      title: t('fb_title'), subtitle: t('fb_sub'),
      actions: React.createElement(FBtn, { variant: 'primary', iconLeft: React.createElement(MIcon, { name: 'plus', size: 18 }), onClick: openNew }, t('fb_log')),
    }),
          ...
          shown.map((f) => {
            ...
            return React.createElement('div', { key: f.id, className: 'lrow', style: { alignItems: 'flex-start' } },
              ...
              React.createElement('div', { className: 'lrow__actions' },
                React.createElement(FIB, { label: f.status === 'done' ? t('fb_reopen') : t('fb_resolve'), size: 'sm', onClick: () => toggleDone(f) }, React.createElement(MIcon, { name: 'check', size: 16 })),
                React.createElement(FIB, { label: t('edit'), size: 'sm', onClick: () => setModal({ ...f }) }, React.createElement(MIcon, { name: 'edit', size: 16 })),
                React.createElement(FIB, { label: t('delete'), size: 'sm', onClick: () => remove('feedback', f.id) }, React.createElement(MIcon, { name: 'trash', size: 16 })),
              ),
            );
          }),
        )
      : React.createElement(FC, null, React.createElement(Empty, { icon: 'message', title: t('fb_none_title'), sub: t('fb_none_sub') })),
