...
function Modal({ open, onClose, title, children, footer, width = 520 }) {
  ...
  return React.createElement(
    ...
    React.createElement(
      'div',
      { className: 'm-dialog', style: { maxWidth: width }, role: 'dialog', 'aria-modal': 'true' },
      React.createElement(
        'div',
        { className: 'm-dialog__head' },
...
function Select({ label, value, onChange, options, hint }) {
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        'select',
        { className: 'm-select__el', value, onChange: (e) => onChange(e.target.value) },
        options.map((o) => {
    ...
    React.createElement(
      ...
      PALETTE.map((p) =>
        React.createElement(
          ...
          value === p.id &&
            React.createElement(MIcon, { name: 'check', size: 16, style: { color: '#fff' } }),
        ),
      ),
    ),
  );
...
function Empty({ icon = 'sparkle', title, sub, action }) {
  return React.createElement(
    ...
    React.createElement(
      'div',
      { className: 'm-empty__icon' },
      React.createElement(MIcon, { name: icon, size: 28 }),
    ),
