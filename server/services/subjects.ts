...
export async function list(db: Db): Promise<SubjectRow[]> {
  const rows = await db.select().from(subjects).orderBy(asc(subjects.sortOrder), asc(subjects.name));
  return rows.map(map);
}
...
export async function update(db: Db, id: string, patch: Partial<SubjectInput>): Promise<SubjectRow> {
