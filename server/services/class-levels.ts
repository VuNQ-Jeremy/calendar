...
export async function list(db: TenantDb): Promise<ClassLevelRow[]> {
  const rows = await db.raw
    .select()
    .from(classLevels)
    .where(db.own(classLevels))
    .orderBy(asc(classLevels.sortOrder), asc(classLevels.name));
  return rows.map(map);
}

export async function create(db: TenantDb, input: ClassLevelInput): Promise<ClassLevelRow> {
  ...
  const rows = await db.raw
    .select()
    .from(classLevels)
    .where(db.own(classLevels, eq(classLevels.id, id)));
...
export async function remove(db: TenantDb, id: string): Promise<void> {
  ...
  await db.delete(classLevels, eq(classLevels.id, id));
}
...
export async function reorder(db: TenantDb, ids: string[]): Promise<void> {
  for (let i = 0; i < ids.length; i++) {
    await db.update(classLevels, { sortOrder: i + 1 }, eq(classLevels.id, ids[i]));
  }
...
export async function update(
  ...
  if (Object.keys(set).length) {
    await db.update(classLevels, set, eq(classLevels.id, id));
  }
  const rows = await db.raw
    .select()
    .from(classLevels)
