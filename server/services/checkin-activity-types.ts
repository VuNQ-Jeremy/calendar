...
export async function list(db: Db): Promise<ActivityTypeRow[]> {
  const rows = await db
    .select()
...
export async function create(db: Db, input: CheckinActivityTypeInput): Promise<ActivityTypeRow> {
  ...
  const rows = await db.select().from(checkinActivityTypes).where(eq(checkinActivityTypes.id, id));
...
export async function remove(db: Db, id: string): Promise<void> {
  ...
  await db.delete(checkinActivityTypes).where(eq(checkinActivityTypes.id, id));
}
...
export async function reorder(db: Db, ids: string[]): Promise<void> {
  for (let i = 0; i < ids.length; i++) {
    await db
      .update(checkinActivityTypes)
      .set({ sortOrder: i + 1 })
      .where(eq(checkinActivityTypes.id, ids[i]));
  }
...
export async function update(
  ...
  const beforeRows = await db
    .select()
    .from(checkinActivityTypes)
    .where(eq(checkinActivityTypes.id, id));
  ...
  if (Object.keys(set).length) {
    await db.update(checkinActivityTypes).set(set).where(eq(checkinActivityTypes.id, id));
  }
  const rows = await db.select().from(checkinActivityTypes).where(eq(checkinActivityTypes.id, id));
