 ...
 * class competes in on /rankings. Its own file rather than growing crud-config, matching the
 * existing crud-config2 split. The two seeded rows (Cơ bản / Nâng cao) are re-asserted by
 * scripts/test-accounts.sql on every reset and must be left alone: the class and rankings specs
 * pick them from the class form.
 */

test.describe('CRUD: class levels', () => {
  crudGuard();
  ...
  test.beforeEach(async ({ page }) => {
    ...
    test('subject: create, rename, deactivate, delete', async ({ page }) => {
      ...
      const card = page.locator('.mochi-card', {
        has: page.getByRole('heading', { name: 'Subjects' }),
      });
  ...
  test('class level: create, rename, deactivate, delete', async ({ page }) => {
    ...
    const card = page.locator('.mochi-card', {
      has: page.getByRole('heading', { name: 'Class levels' }),
    });
