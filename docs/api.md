...
| GET PATCH | `/api/garden/plant` | **user** | The caller's own plant, settled to today; staff may read anyone's with `?studentId=`. PATCH takes `PlantPatchInput` (`plantName`, `potColor`, `species`) and is students-only, on their own plant. `species` (see `shared/garden-art.ts`) is only accepted while the plant is unplanted, dead, or still a seed — the state a harvest leaves it in — and only for a species unlocked by lifetime fruit; otherwise 409 `growing` / `locked` / `unknown_species`. Also returns `today` (the server's ICT day — compare every deadline against it, never the device clock), `hasPlant`, and `fruitMonth` |
...
| GET | `/api/practice/my` | **user** | Practice (Nhiệm vụ), student-only: today plus the next seven days, each enabled class's month summary and pending excuses, and the server's own ICT date — the phone must draw the deadline the nightly job will apply, not its own |
| POST | `/api/practice/submit` | **user** | Student-only, **multipart/form-data**: `studentTaskId`, optional `timeFrom`/`timeTo`/`note`, and an optional `file` part (jpeg/png/webp/mp4/quicktime, ≤ 50 MB). Overwrites a previous submission when resubmitting after a rejection. 409 `deadline_passed` once the practice day is over |
| POST | `/api/practice/excuse` | **user** | Student-only, `PracticeExcuseRequestInput` — asks to be excused for one practice day. Only before that day ends; afterwards only a teacher can excuse the resulting miss (409 `deadline_passed`) |
