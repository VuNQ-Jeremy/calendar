...
| File | Responsibility |
|---|---|
| `shared/walkthrough.ts` | `TourTarget`/`TourStep`/`TourStory`/`TourJourney` types, `JOURNEYS` (7), `STORIES` (29 — `practice-plan-week` and `practice-review-ledger` joined the `content` journey with the Nhiệm vụ feature on 2026-09-04), `WT_PREFIX = 'WALKTHROUGH'`. PURE — no React/DOM/`server/` imports (rule stated in its header; `mobile/` consumes `shared/` as a `file:` dep). Story text is deliberately English-only. |
...
| `app/routes/_app.tsx` (~30, ~500) | Driver mount, since `3917872`: `React.lazy` import + `{user.kind === 'staff' && isTourWindow && <Suspense>…}` where `isTourWindow` reads `?tour=` once post-hydration — visibility/perf gate only; the route guard is the permission. |
